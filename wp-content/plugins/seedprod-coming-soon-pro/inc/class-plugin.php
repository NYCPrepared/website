...
class SEED_CSP3_PLUGIN extends SEED_CSP3 {
	...
	function __construct(){
            ...
            // Check License
            add_action( 'wp_ajax_seed_csp3_check_license', array(&$this,'check_license'));
    ...
    function check_license(){
        if(check_ajax_referer('seed_csp3_check_license')){
            $api_key = $_GET['apikey'];
            $data = array(
                    'action' => 'info', 
                    'slug' => 'seedprod-coming-soon-pro/seedprod-coming-soon-pro.php',
                    'domain' => home_url(),
                    'api_key' => $api_key,
                    'installed_version' => SEED_CSP3_VERSION,
                    );

            $response = wp_remote_post( SEED_CSP3_API_URL, array(
                'method' => 'POST',
                'timeout' => 45,
                'body' => $data,
                )
            ...
            if( is_wp_error( $response ) ) {
               ...
               exit();
                ...
                $s = get_option('seed_csp3_settings_1');
                $s = maybe_unserialize($s);
                if(is_array($s)){
                    $s['api_key'] = $api_key;
                    update_option('seed_csp3_settings_1', $s);
                }
                update_option('seedprod-coming-soon-pro_update_msg',$response->message);
            }
    ...
    function subscribe_callback(){
        ...
        switch ($emaillist) {
            ...
            case 'sendy':
                ...
                $args = array('timeout' => 45,'body' => array('name'=>$name,'email'=>$email,'list'=> $list,'boolean'=>true));
                ...
                if ( is_wp_error( $response ) ) {
                    ...
                    if($body == '1'){
                        $seed_csp3_post_result['status'] = '200';
                    }elseif($body == 'Already subscribed.'){
                        $seed_csp3_post_result['status'] = $body;
                    }
